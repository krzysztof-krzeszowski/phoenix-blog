defmodule Pxblog.CommentHelper do
  ...
  defp get_post(post_id), do: Repo.get!(Post, post_id) |> Repo.preload([:user, :comments])

  defp is_authorized_user?(user, post), do: (user && (user.id == post.user_id  || RoleChecker.is_admin?(user)))
end
